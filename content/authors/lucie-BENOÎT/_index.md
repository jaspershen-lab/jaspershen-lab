---
bio: Biostatistics, Microbiome, Mathematics Modelling, Bioinformatics, Population Health, Multi-omics.
education:
  courses:
  - course: French preparatory cycle in biology and digital sciences
    institution: ISEN Lille
    year: 2022-2024
email: "lucie.benoit@efrei.net"
commentable: true
first_name: Lucie
last_name: Benoit
highlight_name: true
interests:
- 🖥 Bioinformatics
- 💊 Drug design 
organizations:
- name: ISEN Lille
  url: ""
role: Intern Master student (2025-)
social:
# - icon: house
#   icon_pack: fas
- icon: envelope
  icon_pack: fas
  link: lucie.benoit@efrei.net
- display:
    header: false
  icon: github
  icon_pack: fab
  # label: Follow us on GitHub
  link: https://github.com/lucie-benoit
- display:
    header: false
# - icon: cv
#   icon_pack: ai
#   link: https://github.com/qqqq414/File/blob/main/Resume_Qi%20Qi.pdf
status:
  icon: ☕️
superuser: false
title: Lucie BENOIT
user_groups:
- Students
- Graduate Students
---

“The important thing is to never stop questioning,” said Einstein. Guided by this mindset, I have developed a strong interest in the pharmaceutical field and drug discovery. My current focus is on applying bioinformatics and language modeling in drug design.